...
🛠 開発ルール
モデルの変更: my_project_server/lib/src/protocol/ 内の .yaml ファイルを変更した後は、必ず serverpod generate を実行してください。

DBスキーマの変更: モデルの変更をDBに反映させるには、serverpod create-migration を実行した後、サーバーを --apply-migrations 付きで再起動します。
